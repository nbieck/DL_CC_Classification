...
To compare the model with our method, we used FC4\cite{hu2017fc}.
This fully convolutional neural network estimates the image's global illumination color and converts the normalized global illumination with canonical light source color.

We will utilize this model with vgg16 as comparison with our classifiers.
We are using this model because it has achieved higher color correction accuracy than the present state-of-the-art method. Additionally, FC4 learns the informative area for color constancy throughout the image. This lets the model to have no limitation to the possible range of illumination colors, which we considered this model to be the best practice to implementing before the vgg16 convolutional neural network architecture for the comparison with our classification models.
